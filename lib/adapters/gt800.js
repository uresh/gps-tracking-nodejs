...
exports.protocol = 'GT800';
exports.model_name = 'GT800';
exports.compatible_hardware = ['GT800/supplier'];
  ...
  *******************************************/
  this.parse_data = function (data) {
    ...
    if (parts['start'] == '7878') {
      ...
      if (parts['protocal_id'] == '01') {  //Login Information
        parts['device_id'] = data.substr(8, 16);
        parts.cmd = 'login_request';
        parts.action = 'login_request';
      } else if (parts['protocal_id'] == '22') {  //Positioning Data（UTC） 
        parts['device_id'] = '';
        parts['data'] = data.substr(8, parts['length'] * 2);
        parts.cmd = 'ping';
        parts.action = 'ping';
      } else if (parts['protocal_id'] == '13') {  //Heartbeat Packet
        parts['device_id'] = '';
        parts.cmd = 'heartbeat';
        parts.action = 'heartbeat';
      } else if (parts['protocal_id'] == '26') {  //Alarm Data（UTC
        parts['device_id'] = '';
        parts['data'] = data.substr(8, parts['length'] * 2);
        parts.cmd = 'alert';
        parts.action = 'alert';
      } else if (parts['protocal_id'] == '21' || parts['protocal_id'] == '15' ) {  //Online Command Response of Terminal
        parts['device_id'] = '';
        parts['data'] = data.substr(8, parts['length'] * 2);
        parts.cmd = 'terminal_response';
        parts.action = 'terminal_response';
      } else if (parts['protocal_id'] == '8a') {  //Time Check Packet
        parts['device_id'] = '';
        parts['data'] = data.substr(8, parts['length'] * 2);
        parts.cmd = 'timecheck';
        parts.action = 'timecheck';
      }else {
        parts['device_id'] = '';
        parts.cmd = 'noop';
        parts.action = 'noop';
  ...
  this.get_ping_data = function (msg_parts) {
    ...
    var data = {
      ...
      'longitude': this.dex_to_degrees(str.substr(22, 8)),
      'speed': parseInt(str.substr(30, 2), 16),
      'orientation': this.dex_to_degrees(str.substr(32, 4)),
